...
TexturedSpline* TexturedSpline::create(PointArray *config, float tension,  unsigned int segments, const Color4F &color, unsigned int subvisionLevel, const std::string& imagePath, InterpolationType type)
{
    auto instance = new (std::nothrow) TexturedSpline();
    if(instance && instance->init1(config, tension, segments, subvisionLevel, imagePath, color, type))
    {
...
void TexturedSpline::setPath(PointArray *config, float tension,  unsigned int segments, bool is_build)
{
    path.clear();
    ssize_t p;
    float lt;
...
void TexturedSpline::setPathColor(const Color4F &color)
{
    //颜色
    _color = color;
}

//TexturedSpline* TexturedSpline::create(std::vector<cocos2d::Vec2> path, unsigned int subvisionLevel, const std::string& imagePath, const Color4F &color, InterpolationType type)
//{
//	auto instance = new (std::nothrow) TexturedSpline();
//	if(instance && instance->init1( path, subvisionLevel, imagePath, color, type))
//	{
//		return instance;
//	}
//	CC_SAFE_DELETE(instance);
...
//}
//
//TexturedSpline* TexturedSpline::create(std::vector<cocos2d::Vec2> path, unsigned int subvisionLevel, Texture2D* texture, const Color4F &color, InterpolationType type)
//{
//	auto instance = new (std::nothrow) TexturedSpline();
//	if(instance && instance->initWithTexture( path, subvisionLevel, texture, color, type))
//	{
//		return instance;
//	}
//	CC_SAFE_DELETE(instance);
...
bool TexturedSpline::init1(cocos2d::PointArray *config, float tension,  unsigned int segments, unsigned int subvisionLevel, const std::string& imagePath, const cocos2d::Color4F &color, InterpolationType type)
{
	auto textureCache = Director::getInstance()->getTextureCache();
	auto texture = textureCache->addImage(imagePath);
	return initWithTexture(config, tension, segments, subvisionLevel, texture, color, type);
}

bool TexturedSpline::initWithTexture(cocos2d::PointArray *config, float tension,  unsigned int segments, unsigned int subvisionLevel, Texture2D* texture, const cocos2d::Color4F &color, InterpolationType type)
{
    setPath(config, tension, segments, false);
...
//		CCLOGERROR(errorMsg);
	...
	//set default shader
	setGLProgramState(GLProgramState::getOrCreateWithGLProgramName(GLProgram::SHADER_NAME_POSITION_TEXTURE_SP));
...
void TexturedSpline::onDraw(const Mat4 &transform, uint32_t flags)
{
    ...
    getGLProgram()->setUniformLocationWith4f(getGLProgram()->getUniformLocation("sp_color"), _color.r, _color.g, _color.b, _color.a);
    getGLProgram()->setUniformLocationWith1f(getGLProgram()->getUniformLocation("sp_opacity"), getOpacity()/255.f);
